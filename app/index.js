...
let canvasThree = document.getElementById('canvas-3');
let elem = document.getElementById("message-box");

const boosterImages = [];
const boosters = [];
boosters[0] = new Booster(2, 10, 10000);
boosters[1] = new Booster(3, 50, 20000);
boosters[2] = new Booster(4, 100, 10000);
boosters[3] = new Booster(5, 500, 20000);
boosters[4] = new Booster(6, 750, 25000);
boosters[5] = new Booster(1, 0, 0);
boosters[6] = new Booster(1, 0, 0);
boosters[7] = new Booster(1, 0, 0);
boosters[8] = new Booster(1, 0, 0);

const bosses = [];
bosses[0] = new Boss("homophobes", 250, "boss.gif");
bosses[1] = new Boss("bestboro waptist church", 250, "boss.gif");
bosses[2] = new Boss("bestboro waptist church", 250, "boss.gif");


let currentBoss = bosses[level-1];
let hp = currentBoss.hp;
console.log(currentBoss.hp);
...
let posY = 0;
	...
	let images = [	'gayagenda-pixel.png', 'rainbow-potion-pixel.png', 'drag-disguise-pixel.png', 'horn-pixel.png',
			'fairy-pixel.png', 'empty.jpg', 'empty.jpg', 'empty.jpg', 'empty.jpg'];
	...
	function handleMouseDown(e) {
		...
		let activateBooster = (num) => {
				...
				boosters[num].activated = true;
					while(boosters[num].activated){
						...
						setTimeout(function(){
										...
										context.fillStyle = 'rgba(225,225,225,0.5)';
										//context.clearRect(boxes[num].x,boxes[num].y, boxes[num].width, boxes[num].height);
										let background = new Image();
										let img = new Image();
			...
			function handleMouseUp(e) {
			    e.stopPropagation();
			    $("#canvas-1").off("mouseup", handleMouseUp); //remove mouse up handler
...
function saveGame(){
	...
	let save = {
		//level: currentLevel
		points: points
		//boosters: //*array of current boosters*
	//	bossHP: bossHP
	}
	try {
